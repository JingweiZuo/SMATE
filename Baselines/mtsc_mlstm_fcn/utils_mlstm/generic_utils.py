...
from utils_mlstm.constants import TRAIN_FILES, TEST_FILES, MAX_NB_VARIABLES, NB_CLASSES_LIST
...
def load_dataset_at(index, fold_index=None, normalize_timeseries=False, verbose=True) -> (np.array, np.array):
    if type(index) == str:
        ...
        train_path = TRAIN_FILES[index]
        test_path = TEST_FILES[index]
    ...
    if fold_index is None:
        ...
        x_train_path = train_path + "X_train_%d.npy" % fold_index
        y_train_path = train_path + "y_train_%d.npy" % fold_index
        x_test_path = test_path + "X_test_%d.npy" % fold_index
        y_test_path = test_path + "y_test_%d.npy" % fold_index
