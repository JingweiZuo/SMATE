...
from utils_mlstm.generic_utils import load_dataset_at, calculate_dataset_metrics, cutoff_choice, \
                                cutoff_sequence
from utils_mlstm.constants import MAX_NB_VARIABLES, MAX_TIMESTEPS_LIST
...
def train_model(model:Model, dataset_id, dataset_prefix, dataset_fold_id=None, epochs=50, batch_size=128, val_subset=None,
    ...
    y_train_test = to_categorical(y_train_test, len(np.unique(y_train)))
    y_train = y_train_test[: y_train.shape[0]]
    y_test = y_train_test[y_train.shape[0] :]
    if is_timeseries:
        factor = 1. / np.cbrt(2)
    else:
        factor = 1. / np.sqrt(2)
    ...
    model.fit(X_train, y_train, batch_size=batch_size, epochs=epochs, callbacks=callback_list, class_weight=class_weight, verbose=0)#, validation_data=(X_test, y_test)
    #model.fit(X_train, y_train, batch_size=batch_size, epochs=epochs, callbacks=callback_list, class_weight=class_weight, verbose=2, validation_split=0.1)
...
def evaluate_model(model:Model, dataset_id, dataset_prefix, dataset_fold_id=None, batch_size=128, test_data_subset=None,
    ...
    print("Final Accuracy %.4d : ", accuracy)
